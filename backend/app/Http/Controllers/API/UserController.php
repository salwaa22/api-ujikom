...
use App\Http\Requests\User\StoreUserRequest; use App\Http\Requests\User\UpdateUserRequest; 
    ...
    { 
        ...
        $user = DB::transaction(function () use ($request, $data) { 
            ...
            if ($request->hasFile('foto_profile')) { 
                $data['foto_profile'] = 
$request->file('foto_profile')->store('profiles', 'public'); 
            } 
...
JsonResponse 
    { 
        ...
        DB::transaction(function () use ($request, $data, $user) { 
            if ($data->get('password')) {$data['password'] = Hash::make($data['password']); 
            } else { 
