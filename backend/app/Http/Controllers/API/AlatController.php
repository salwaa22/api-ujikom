...
use App\Http\Resources\AlatResource; use App\Models\Alat; 
...
{ 
    ...
    { 
        ...
        $alat = DB::transaction(function () use ($request, $data) { 
            if ($request->hasFile('gambar')) {                 $data['gambar'] = $request->file('gambar')->store('alat', 
'public'); 
            } 
...
JsonResponse 
    { 
        ...
        DB::transaction(function () use ($request, &$data, $alat, 
$oldGambar) { 
            if ($request->hasFile('gambar')) {                 $data['gambar'] = $request->file('gambar')->store('alat', 
'public');                 
