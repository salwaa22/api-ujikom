                    ...
                    @forelse($alat as $item)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="py-3 px-4 border-b">
                                @if($item->gambar)
                                    <img src="{{ asset($item->gambar) }}" alt="{{ $item->nama_alat }}" class="w-12 h-12 object-cover rounded-lg border">
                                @else
                                    <span class="text-xs text-gray-400 italic">Tidak ada</span>
                                @endif
                            </td>
                            <td class="py-3 px-4 border-b font-medium text-gray-900">{{ $item->nama_alat }}</td>
                            <td class="py-3 px-4 border-b">{{ $item->kategori->nama_kategori ?? '-' }}</td>
                            <td class="py-3 px-4 border-b font-semibold">{{ $item->stok }}</td>
                            <td class="py-3 px-4 border-b">
                                <span class="px-2.5 py-1 text-xs font-semibold rounded-full 
                                    @if(strtolower($item->status_kondisi) == 'baik') bg-emerald-100 text-emerald-800 
                            ...
                            <td class="py-3 px-4 border-b">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.alat.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus alat ini?')">
                                        @csrf
                                        @method('DELETE')
